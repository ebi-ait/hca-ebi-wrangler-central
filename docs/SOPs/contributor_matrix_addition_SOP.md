...
For each expression matrix or cell type annotation file that is found, a row needs to be filled in the metadata spreadsheet, in the ‘Analysis file’ tab. Analysis files can be linked to sequence file or biomaterial entities via the ‘Analysis_protocol’ entity (see the ‘Analysis protocol’ tab). This functions similar to a ‘sequencing protocol’ to link different entities. 
...
The previous process here is outdated, but kept for recording: 
