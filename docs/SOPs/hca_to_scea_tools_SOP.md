...
- The dataset consists of data generated from at most 1 single-cell RNA sequencing technology type. If the data is derived from more than 1 single-cell sequencing type, the dataset must be split into datasets separated by technology type and with separate E-HCAD-ids. In practise, this would mean the hca2scea tool should be run twice, once for each spreadsheet obtained when the dataset is split by technology type. Datasets with multiple 10X versions can be submitted as a single dataset, but the hca2scea tool must be run separately for each version. The resulting data files can then be merged manually to include the different 10X versions.

- Bulk RNA sequencing libraries are not eligible.

- The hca-to-scea tool does not currently support the 10X Visium technology type (or other spatial data).
...
- If the dataset consists of organoid or iPSC sample types, the dataset is eligible as input to the hca2scea tool so long as all cell suspensions are derived from the same biomaterial type. If there are multiple input biomaterial types, the dataset is eligible for scea but the dataset must be split by the input biomaterial type before running the hca2scea tool separately. The output files can then be manually merged before submission to scea.

- Cell lines other than iPSC, hESC or organoid are not eligible e.g. 2D immortalised cell lines.

- The hca2scea tool does not process pooled biomaterials (e.g. pooled donors, specimens or cell suspensions).

### Protocol type eligibility criteria

- Experiments involving cell-type enrichment using cell markers are eligible. However, the script does not currently support the addition list of the sample cell marker status. This should be added manually using a "Characteristics[immunophenotype]" column in the sdrf file.

- Experiments involving treatment with a drug, stimulus or other treatment type are eligible. However, the script does not currently support the addition of this protocol type or treatment status. The treatment protocol sbould be labelled as a "growth protocol" in the idf file. A Protocol REF column in the sdrf file can then be used to record which samples were subjected to the treatment protocol.

### Data availability criteria

- The full path to fastq files or SRA object files should be available for the dataset run accessions. The script will attempt to find these and input them in the sdrf file automatically. Datasets with only raw data in bam file format are not currently eligible.
...
- The hca2scea tool does not accept pooled biomaterials (e.g. pooled donors, specimens or cell suspensions). Pooled samples must be removed from the input spreadsheet for the hca2scea tool to run successfully.

### Protocol type

- Spreadsheets involving cell-type enrichment using cell markers are eligible. However, the script does not currently support the addition list of the sample cell marker status.

- Spreadsheets involving treatment with a drug, stimulus or other treatment type are eligible. However, the script does not currently support the addition of this protocol type or treatment status.
